...
\begin{itemize}
	...
	\item Generador de señales: Equipo utilizado para excitar el circuito RC con señales periódicas de amplitud controlada (principalmente sinusoidal) y frecuencia ajustable. Durante la práctica se mantuvo la amplitud constante y se varió la frecuencia para caracterizar la respuesta en magnitud y fase de los filtros pasa-baja y pasa-alta.
	\item Multímetro digital: Instrumento para medir resistencia, capacitancia y voltaje, según la configuración empleada.
...
\begin{enumerate}
    \item Se conectaron los instrumentos de medición: el osciloscopio para visualizar las señales de entrada y salida, el generador de señales para proporcionar la señal de prueba, y el multímetro para verificar los valores de los componentes. Se armó el circuito pasa-baja RC según la figura \ref{fig:circuito-pasa-baja} cambiando los valores de la resistencia y el capacitor a 10 k$\Omega$ y 3,98 nF, respectivamente.
    \begin{figure}[H]
        \centering
...
\begin{enumerate}
	\item Se armó el circuito pasa-alta RC según la Figura \ref{fig:circuito-pasa-alta} con un capacitor de 3,98 nF y una resistencia de 15,59 k$\Omega$.
	\begin{figure}[H]
        \centering
...
\textbf{Datos reales:}
\begin{itemize}
	\item $R = 10\,\mathrm{k}\Omega$
	\item $C = 3,98\,\mathrm{nF}$
\end{itemize}
...
\begin{table}[H]
	...
	\hline
	\textbf{$V_{out}$ (V)} & \textbf{$f$ (Hz)} & \textbf{$V_{out}/V_{in}$} \\
	\hline
	$4,51$ & $2000$ & $0,902$ \\
	\hline
	$4,28$ & $2500$ & $0,856$ \\
...
\textbf{Datos reales:}
\begin{itemize}
	\item $R \approx 19,6\,\mathrm{k}\Omega$
	\item $C = 3,98\,\mathrm{nF}$
\end{itemize}
